<?php namespace KODESK\Includes\Classes;
/**
 * Common functions
 */
class Common {
	...
	function blog( $template = 'blog' ) {
		...
		if ( ( $wp_query->is_posts_page && 'blog' == $template ) || $template == 'page' ) {
			...
			$return = [
				'layout'            => kodesk_meta( 'sidebar_sidebar_layout', $page_id, 'right' ),
				'sidebar_type'      => kodesk_meta( 'sidebar_source_type', $page_id, 'd' ),
				'sidebar_elementor' => kodesk_meta( 'sidebar_elementor_template', $page_id ),
				'sidebar'           => kodesk_meta( 'sidebar_page_sidebar', $page_id, 'default-sidebar' ),
				'banner_type'       => kodesk_meta( 'banner_source_type', $page_id ),
				'banner_elementor'  => kodesk_meta( 'banner_elementor_template', $page_id ),
				'banner'            => kodesk_set( kodesk_meta( 'banner_page_background', $page_id ), 'url' ),
				'title'             => kodesk_meta( 'banner_banner_title', $page_id ),
				'enable_banner'     => kodesk_meta( 'banner_page_banner', $page_id ),
			];
			...
			$banner_image  = $template . '_banner_image';

			$return = [
				'enable_banner' => $options->get( $enable_banner ),
				'title'         => $options->get( $title ) ? $options->get( $title ) : kodesk_the_title( $template ),
				'banner'        => kodesk_set( $bg, 'url' ),
				'banner_image'  => kodesk_set( $banner_image, 'url' ),
